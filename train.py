...
def train(load_path=None, save=False, epochs=4, full_dataset=True):
    ...
    model.compile(
        ...
        metrics={
            'note': [
                'accuracy',
                # PolyphonyAccuracy(name='polyphony_accuracy'),
                tf.keras.metrics.Precision(name='precision'),
                tf.keras.metrics.Recall(name='recall'),
                # tf.keras.metrics.F1Score(name='note_f1_score')
            ],
            'onset': [
                'accuracy',
                tf.keras.metrics.Precision(name='precision'),
                tf.keras.metrics.Recall(name='recall'),
                # tf.keras.metrics.F1Score(name='onset_f1_score')
            ]
        }
    )
...
if __name__ == "__main__":
    ...
    # load_path = None
    load_path = '/Users/tomasz/Documents/Learning/Studies/zzMasters/Thesis/perfect-peach/pp1/saved_models/2506070036_model_epoch_02_08.keras'
    train(load_path=load_path, save=True, epochs=10)